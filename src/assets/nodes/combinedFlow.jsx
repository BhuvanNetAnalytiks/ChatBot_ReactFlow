...
import StartNodeJson from '../../data/StartNode.json';
import departmentDetectionJson from '../../data/departmentDetection.json';
import IdentityProviderJson from '../../data/identityProvider.json';
import GetDetAzure from '../../data/getDetails.json';
import MilvusDataJson from '../../data/milvusDatabase.json';
import Financenode from './financeNode';
import Sidebar from './sidebar';
import voiceAgent from './voiceAgent';
...
// Initial nodes setup extracted to a constant
const initialNodes = [
    {
        name: 'GREETING ',
        description: 'The greeting  is responsible for generating a greeting message to the user',
...
const CombinedFlow = () => {
    ...
    useEffect(() => {
        try {
            const savedNodes = localStorage.getItem('react-flow-nodes');
            const savedEdges = localStorage.getItem('react-flow-edges');
            console.log('Loaded from localStorage - Nodes:', savedNodes);
            console.log('Loaded from localStorage - Edges:', savedEdges);

            if (savedNodes && savedEdges) {
                const parsedNodes = JSON.parse(savedNodes);
                const parsedEdges = JSON.parse(savedEdges);
                if (parsedNodes.length > 0 && parsedEdges) {
                    console.log('Setting nodes and edges from localStorage');
                    ...
                    console.log('Saved state is empty, initializing with defaults');
    ...
    useEffect(() => {
        try {
            if (nodes.length > 0) {
                ...
                localStorage.setItem('react-flow-nodes', JSON.stringify(nodes));
            }
        } catch (error) {
    ...
    useEffect(() => {
        try {
            if (edges.length > 0) {
                ...
                localStorage.setItem('react-flow-edges', JSON.stringify(edges));
            }
        } catch (error) {
    ...
    const syncDepartmentNodes = useCallback(() => {
        ...
        currentDepartments.forEach((dept, index) => {
            const deptNormalized = dept.trim();
            if (!existingDeptNames.includes(deptNormalized)) {
                const nodeType = departmentNodeTypes[deptNormalized.toUpperCase()] || 'DynamicNode';
                const newNodeId = `${deptNormalized.toLowerCase()}node-${Date.now() + index}`;
                const newNode = {
                    id: newNodeId,
                    type: nodeType,
    ...
    const saveCombinedGraphToFile = useCallback(async () => {
        const stepMapping = {
            ...
            ticketingNode: (node) => {
                if (node.data.selected === 'ServiceNow') {
                    return {
                        create: serviceNowCreateJson.steps[0],
                        view: serviceNowViewJson.steps[0],
                    };
                } else if (node.data.selected === 'Jira') {
                    return {
                        create: jiraCreateJson.steps[0],
                        view: jiraViewJson.steps[0],
                    };
                } else if (node.data.selected === 'Zendesk') {
                    return {
                        create: zendeskCreateJson.steps[0],
                        view: zendeskViewJson.steps[0],
                    };
                }
                return null;
            },
